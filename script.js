...
function triggerConfetti() {
    gameState = 'WON'; // Change state to stop game logic but allow drawing
    particles = []; // Clear existing particles

    for (let i = 0; i < 150; i++) {
        setTimeout(() => {
            const p = new Particle(Math.random() * canvas.width, -20);
            p.color = `hsl(${Math.random() * 360}, 70%, 60%)`;
            p.speedY = Math.random() * 5 + 2;
            p.speedX = (Math.random() - 0.5) * 10;
            p.life = 200; // Longer life for confetti
            p.size = Math.random() * 8 + 4;
            particles.push(p);
        }, i * 10);
    }

    // Restart loop for celebration
    updateGame();
}
